...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // Let the useEffect handle the redirect based on profile existence.
    } catch (err: any) {
      ...
      <div className="flex-1 flex flex-col items-center justify-center py-20">
        <div className="w-full max-w-md px-6">
          ...
          <div className="overflow-hidden rounded-2xl border border-border/70 bg-surface/50 p-8 backdrop-blur-sm sm:p-10">
            <h1 className="mb-2 text-2xl font-bold tracking-tight text-center">Welcome back</h1>
            <p className="mb-8 text-sm text-muted text-center">Sign in to your GDGoC RIT account</p>
            
            {error && <div className="mb-6 rounded-md bg-accent-red/10 p-3 text-sm text-accent-red text-center">{error}</div>}
